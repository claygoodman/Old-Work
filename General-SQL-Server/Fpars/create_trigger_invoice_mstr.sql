...
CREATE      TRIGGER [dbo].[TR_fpars_invoice_mstr]
ON [Fpars].[dbo].[invoice_mstr_2008_Q1]
AFTER UPDATE 
AS 
    ...
    select convert(integer, substring(convert(varchar, getdate(), 112),1,6)) as partn_key,
           i.invhdr_upd_work_id as work_id,
           ...
           -1 as rule_id
      from deleted d INNER JOIN inserted i ON d.invoice_id = i.invoice_id
    ...
    select convert(integer, substring(convert(varchar, getdate(), 112),1,6)) as partn_key,
           ...
           'INVOICE_MSTR', 
           i.invoice_id, 
           'PURCHASE_ORDER_NO' as chg_column,
           d.PURCHASE_ORDER_NO as chg_orig_value,
           i.PURCHASE_ORDER_NO as chg_new_value,
           'UPDATED',
           'Column Updated',
           NULL as chg_comment, 
           NULL as chg_trigger_comment, 
           -1 as rule_id
      from deleted d INNER JOIN inserted i ON d.invoice_id = i.invoice_id
     where upper(d.PURCHASE_ORDER_NO) <> upper(i.PURCHASE_ORDER_NO)
       and d.PURCHASE_ORDER_NO is not null
